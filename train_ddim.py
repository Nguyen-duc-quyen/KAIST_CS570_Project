...
def main(cfg):
    ...
    train_transforms = build_transforms(cfg.train_set.transforms)
    train_dataset = build_dataset(cfg.train_set.dataset_type)
    train_dataset.transform = train_transforms
    train_loader = build_dataloader(cfg.train_set, dataset=train_dataset)
    ...
    print("[INFO]: Train dataset size: ", len(train_loader.dataset))
    print("[INFO]: Validation dataset size: ", len(val_loader.dataset))

    # Create one uniform Lightning Data Module
    lightning_data_module = GeneralDataModule(
    ...
    trainer = Trainer(
        ...
        logger=loggers,
        check_val_every_n_epoch=10, # Visualize after every epochs
        deterministic=True,
        benchmark=False,
    ...
    os.mkdir(sample_dir)
    if accelerator == "gpu":
        # Move to the first GPU for evaluation
        ddim_trainer.to("cuda:{cfg.devices[0]}")
    else:
        ddim_trainer.to("cpu")
